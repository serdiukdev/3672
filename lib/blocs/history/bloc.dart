...
import 'state.dart';
import '../statistic/state.dart' show SelectedPeriod;
...
class HistoryBloc extends Cubit<HistoryState> {
  ...
  HistoryBloc({String mode = 'expense'}) : super(HistoryState.initial(mode: mode));
  ...
  Future<void> selectPeriod(DateTime start, DateTime end) async {
    ...
    final p = SelectedPeriod(year: state.period.year, month: state.period.month, startDate: a, endDate: b);
    await _savePeriod(p);
    emit(state.copyWith(period: p));
  }
  ...
  Future<void> loadEntries() async {
    ...
    final key = state.mode == 'income' ? _kIncomeEntriesKey : _kExpenseEntriesKey;
  ...
  Future<void> _savePeriod(SelectedPeriod p) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_kSelectedPeriodKey, jsonEncode({
      'year': p.year,
      'month': p.month,
  ...
  Future<SelectedPeriod?> _loadPeriod() async {
    ...
    try {
      ...
      return SelectedPeriod(
        ...
        startDate: (m['startDate'] as String?) != null ? DateTime.tryParse(m['startDate'] as String) : null,
        endDate: (m['endDate'] as String?) != null ? DateTime.tryParse(m['endDate'] as String) : null,
      );
    } catch (_) {
      return null;
    }
  ...
  (DateTime, DateTime) _effectiveRange(SelectedPeriod p) {
    ...
    final start = p.startDate != null ? DateTime(p.startDate!.year, p.startDate!.month, p.startDate!.day) : first;
    final end = p.endDate != null ? DateTime(p.endDate!.year, p.endDate!.month, p.endDate!.day) : last;
