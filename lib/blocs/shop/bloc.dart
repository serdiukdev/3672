...
class ShopBloc extends Cubit<ShopState> {
  ...
  Future<void> load() async {
    ...
    final ownedBaskets = prefs.getStringList(_kOwnedBaskets)?.toSet() ?? <String>{};
    final ownedBackgrounds = prefs.getStringList(_kOwnedBackgrounds)?.toSet() ?? <String>{};
  ...
  Future<bool> purchase(ShopItem item) async {
    ...
    if (item.type == ShopItemType.basket) {
      ...
      if (!ownedBackgrounds.contains(item.id)) {
        ...
        await prefs.setStringList(_kOwnedBackgrounds, ownedBackgrounds.toList());
        if (firstOfType) {
  ...
  Future<void> select(ShopItem item) async {
    if (item.type == ShopItemType.basket && !state.ownedBaskets.contains(item.id)) return;
    if (item.type == ShopItemType.background && !state.ownedBackgrounds.contains(item.id)) return;
