...
class OnboardingBloc extends Bloc<OnboardingEvent, OnboardingState> {
  final List<dynamic> _questions = List<dynamic>.from(onboardingData["questions"] as List);
  ...
  OnboardingBloc({required this.storageService}) : super(PreOnboardingState()) {
    on<StartQuiz>((event, emit) {
      _index = 0;
      emit(QuizQuestionState(index: _index, question: _questions[_index] as Map<String, dynamic>));
    });

    on<SelectAnswer>((event, emit) async {
      final q = _questions[_index] as Map<String, dynamic>;
      emit(QuizQuestionState(index: _index, question: q, selectedId: event.selectedId));
      await Future.delayed(const Duration(milliseconds: 500));
      emit(QuizAnswerState(index: _index, question: q, selectedId: event.selectedId));
    });
    ...
    on<UpdateGoalType>((event, emit) {
      final s = state is PostOnboardingState ? state as PostOnboardingState :  PostOnboardingState();
